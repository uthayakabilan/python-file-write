...
def baseLine():
    ...
    print("read and copy called inside baseline function")
    readAndCopy(dest_dir, dest_file, "w")
    print("read and copy ended inside baseline function ")
...
def runBuild(args):
    if len(args) > 2 and args[1] == "-i":
        ...
        image = args[2]
        print("generate data function started")
        generateData(image)
        print("generate data function ended")
        if len(args) > 3:
            # Extracts run mode from the runtime and compares with the conditions
            run_mode = args[3]
            if run_mode == "baseline":
                print("baseline function start")
                baseLine()
                print("baseline function ended")
